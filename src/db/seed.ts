...
// Issue #29 — schema is owned by runMigrations (versioned migrations),
// applied by the caller before runSeed (the direct-run bootstrap below
// does it; the Next.js app applies it at boot via its db entry point).
 ...
 */
export async function ingestSampleLease(db: Db): Promise<void> {
  ...
  await db
    ...
    .run(SAMPLE_LEASE_ID);
  await db
    .prepare('DELETE FROM clauses WHERE lease_id = ?')
    .run(SAMPLE_LEASE_ID);
  await db.prepare('DELETE FROM leases WHERE id = ?').run(SAMPLE_LEASE_ID);
  ...
  const tempId = await insertLease(db, {
  ...
  await db
    .prepare('UPDATE leases SET id = ? WHERE id = ?')
    .run(SAMPLE_LEASE_ID, tempId);
 ...
 * re-seeds.
 */
export async function ingestCleanSampleLease(db: Db): Promise<void> {
  await db
    .prepare(
      `DELETE FROM negotiation_emails WHERE clause_id IN (
       SELECT id FROM clauses WHERE lease_id = ?
  ...
  await db
    .prepare('UPDATE leases SET id = ? WHERE id = ?')
    .run(SAMPLE_CLEAN_LEASE_ID, tempId);
...
if (require.main === module) {
  (async () => {
    // Issue #29 — seed through the same env-selected backend as the app:
    // hosted Turso when LEASELENS_TURSO_URL is set (`npm run db:seed`
    // targets production this way in `prebuild` on Vercel), otherwise
    // the local file. Schema comes from runMigrations, not the old
    // SCHEMA-exec + migrate() boot path.
    const tursoUrl = env.LEASELENS_TURSO_URL;
    if (!tursoUrl) {
      mkdirSync(dirname(env.LEASELENS_DB_PATH), { recursive: true });
    }
    const db = createDbClient(
      tursoUrl
        ? { url: tursoUrl, authToken: env.LEASELENS_TURSO_AUTH_TOKEN }
        : {
            url:
              env.LEASELENS_DB_PATH === ':memory:'
                ? ':memory:'
                : `file:${env.LEASELENS_DB_PATH}`,
          },
    );
    ...
    try {
      ...
      await ingestCleanSampleLease(db);
