      ...
      {items.length === 0 ? (
        ...
        <ul className="m-0 list-none p-0">
          {items.map((item) => (
            <li
              key={item.id}
              className="grid grid-cols-[180px_minmax(0,1fr)_120px_minmax(0,1fr)] items-center gap-3 border-b border-gray-100 px-4 py-2.5 text-xs"
            >
              ...
              <span className="text-gray-500">{item.approved_by}</span>
              <span className="truncate text-gray-500" title={item.notes ?? ''}>
                {item.notes ?? '—'}
              </span>
            </li>
          ))}
