...
6. Skim `docs/_meta/architecture.md` (descriptive system snapshot — module map, data
   model, sequence flows, design decisions). Read in full when the task requires
   knowing how a subsystem is wired.
7. Confirm in your first response which sprint you are operating in and what
   your current task is, grounded in those files, before touching any code.
...
## 16. The 13-Sprint Roadmap

ContentOps is delivered in 13 sprints (Sprint 0 through Sprint 12). Reframed
from 12 sprints in v1.7 to insert a workspace + brand-onboarding sprint (new
Sprint 11) before the deployment closeout (renumbered to Sprint 12). Prior
v1.6 reframing inserted Sprint 10 (UI Polish Pass).
...
13. **Sprint 12 — Demo Deployment + README + Loom:** Vercel deployment, final documentation, and demo recording.
...
- **v1.9** — Added two companion docs under `docs/_meta/`: `agent-guidelines.md` (prescriptive *how to write code* — stack rules for Next.js 16 App Router / React 19 / better-sqlite3 / Anthropic SDK / Vitest 4 / Tailwind v4, plus ContentOps-specific patterns: workspace_id scoping, audit-row-in-transaction, compensating-action JSON shape, RBAC enforcement points, demo-mode guardrails, lazy TTL purge, cookie ↔ expires_at gray state) and `architecture.md` (descriptive snapshot dated 2026-05-05 — product shape, runtime topology with ASCII diagram, full module map across `src/app/` `src/lib/` `src/components/` `mcp/` `scripts/`, post-Sprint-11 11-table data model with FK / index / workspace-scoping inventory, four sequence flows (chat streaming, RAG retrieval, mutating tool + audit + rollback, workspace upload + ingest + cookie), CSS architecture, testing strategy with current 262/55 vitest counts, 13 key design decisions, deployment shape, 8 known risks). Updated §1 read-first list from 4 to 6 mandatory items + 1 on-demand item, integrating both new docs and explicit governance vs. how-to-code vs. snapshot separation. Adoption pattern borrowed from `docs/_references/ai_mcp_chat_ordo` reviewer assets — ContentOps-grounded, not Phoenix-translated. Architecture-doc refresh discipline: update at sprint boundaries in the same commit as sprint amendments. Authorized by operator session, 2026-05-05.
- **v1.8** — Inter-sprint hardening pass between Sprint 11 and Sprint 12, prompted by an operator-led DB review against `docs/_references/ai_mcp_chat_ordo`. Confirmed the reference is "Pattern source, not code source" — its single-tenant ~30-table schema cannot match ContentOps' workspace-scoped 11-table schema, and shouldn't. Adopted one defensive pattern: `PRAGMA foreign_keys = ON` is now an explicit boot invariant in `src/lib/db/index.ts`, with a boot-state regression assertion in `src/lib/db/schema.test.ts`. Empirically `better-sqlite3@12` already defaults the pragma to `1`, so this is lock-in (against library-default change or a future native-`sqlite3` swap), not a behavioral change. Extended `scripts/diag-db.mjs` to snapshot row counts across all per-data tables and run three FK orphan probes (`chunks→documents`, `messages→conversations`, `conversations→users`); the seeded sample workspace passes all probes at 0 orphans. Fixed a small bootstrap bug in `src/db/seed.ts` where the script opened the DB without `mkdirSync` on its directory, blocking first-run `npm run db:seed`. Deferred items captured for the roadmap: **(1)** add `ON DELETE CASCADE` to the three declared FKs and add `workspace_id REFERENCES workspaces(id) ON DELETE CASCADE` on the six workspace-scoped tables (six Round-4-style table rebuilds; would collapse `src/lib/workspaces/cleanup.ts` to a one-liner), **(2)** FTS5 + BM25 hybrid retrieval to complement vector-only cosine search, both worth a properly-specced sprint via §7. Authorized by operator session, 2026-05-05.
- **v1.7** — Roadmap reframed from 12 sprints to 13 sprints. Pivoted **Sprint 11** from "Demo Deployment + README + Loom" to **"Workspaces & Brand Onboarding"** — ContentOps becomes a workspace-based product where any operator can supply their own brand identity + audience profile rather than being locked to the Side Quest Syndicate sample. Side Quest stays as a one-click sample workspace so reviewers face zero cold-start friction. Adds a `workspaces` table and a `workspace_id` column on existing per-data tables (`documents`, `chunks`, `audit_log`, `content_calendar`, `approvals`); parameterizes the system prompt on the active workspace; adds `/onboarding` and a workspace switcher in the cockpit header; uploaded workspaces TTL after 24h via lazy cleanup. Renumbered the deployment closeout sprint from Sprint 11 to **Sprint 12 — Demo Deployment + README + Loom**. Strategic rationale: the FDE / Applied AI roles ContentOps targets (Doing Things, Distyl, Anthropic FDE, OpenAI FDE) explicitly do customer-engagement onboarding work; "any operator can upload their brand and get a grounded AI workflow in 30 seconds" is the demo story those roles recognize. Demo-grade scope only — no real auth, no PDFs, no LLM-inferred metadata, no multi-tenant isolation beyond `workspace_id` filtering. Authorized by operator brainstorm session, 2026-05-04.
