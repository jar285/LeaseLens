...
/**
 * Lazy TTL cleanup. Runs in a single async transaction:
 ...
 *
 * Every statement goes through the `tx` handle — issuing them on the outer
 * `db` inside the callback would not join the transaction (and throws
 ...
 */
export async function purgeExpiredWorkspaces(db: Db): Promise<PurgeResult> {
  return db.transaction(async (tx): Promise<PurgeResult> => {
 ...
 */
export async function purgeWorkspaceNow(
  db: Db,
  workspaceId: string,
): Promise<PurgeResult> {
  return db.transaction(async (tx): Promise<PurgeResult> => {
    ...
    return purgeWorkspaceIds(tx, [row.id]);
  });
}
 ...
 */
async function purgeWorkspaceIds(
  tx: DbTransaction,
  ids: string[],
): Promise<PurgeResult> {
  ...
  for (const table of WORKSPACE_SCOPED_TABLES) {
    await tx
      .prepare(`DELETE FROM ${table} WHERE workspace_id IN (${placeholders})`)
      .run(...ids);
  }
  ...
  await tx
    ...
    .run(...ids);
  await tx
    .prepare(`DELETE FROM workspaces WHERE id IN (${placeholders})`)
    .run(...ids);
