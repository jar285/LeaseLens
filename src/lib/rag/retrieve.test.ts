...
describe('retrieve', () => {
  ...
  it('returns top-k results for a relevant query', async () => {
    const docId = await seedDocument(db, 'brand-identity');
  ...
  it('respects maxResults option', async () => {
    const doc1Id = await seedDocument(db, 'doc-one');
    const doc2Id = await seedDocument(db, 'doc-two');

    for (let i = 0; i < 3; i++) {
      await seedChunk(db, doc1Id, {
        id: `chunk-a${i}`,
  ...
  it('all returned chunks have rrfScore > 0', async () => {
    const docId = await seedDocument(db, 'style-guide');
    await seedChunk(db, docId, {
      id: 'c1',
  ...
  it('cross-workspace isolation: chunks in workspace A are not returned for a query against workspace B (Sprint 11)', async () => {
    ...
    const docA = await seedDocument(db, 'brand-identity', wsA);
    await seedChunk(db, docA, {
      id: 'chunk-a',
